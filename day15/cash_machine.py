...
class CashMachineCapacity(ValueError):
    pass
...
class CashMachine:
    ...
    def put_money(self, money):
        if len(self._money) > 10:
            raise CashMachineCapacity("BŁĄD: Maksymalna ilość banknotów do wpłaty - 10.")
        else:
...
def main():
    ...
    if operacja == 'w':
        ...
        try:
            wplacamy = bankomat.put_money(banknoty)
        except CashMachineCapacity as e:
            wplacamy = False
            print(e)
        if wplacamy:
...
def test_cash_machine_overpacked():
    cash_machine = CashMachine()
    with pytest.raises(CashMachineCapacity) as e:
        cash_machine.put_money([20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20])